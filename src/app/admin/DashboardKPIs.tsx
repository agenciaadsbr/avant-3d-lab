    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: "0.75rem", marginBottom: "1.5rem" }}>
      {kpis.map(k => (
        <div key={k.label} style={{ backgroundColor: "#fff", border: "1px solid rgba(140,100,20,0.1)", borderRadius: "0.875rem", padding: "0.875rem 1rem", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
          ...
          <div style={{ color: hideProfit ? "#e8e0d0" : "#1a1510", fontSize: "1.15rem", fontWeight: 900, lineHeight: 1 }}>
            {hideProfit ? "—" : k.value}
          </div>
          <div style={{ color: "#9a8060", fontSize: "0.7rem", fontWeight: 700, marginTop: "0.25rem" }}>{k.label}</div>
          {k.sub && <div style={{ color: hideProfit ? "#e8e0d0" : ((k as any).subOk === false ? "#c04040" : "#9a8060"), fontSize: "0.65rem", marginTop: "0.15rem" }}>
            {hideProfit ? "—" : k.sub}
          </div>}
        </div>
      ))}
